...
CMSCommonJs.Form.checkUsername = function(username) {
	...
	if (username) {
		$.ajax({
			url : CMSCommonJs.ROOT_URL + 'user/register/check/user',
			timeout : 10000,
			dataType : "json",
			contentType : "text/plain",
			type : "GET",
...
CMSCommonJs.Form.checkExistUsername = function(username) {
	var output = false;
	if (username) {
		$.ajax({
			url : CMSCommonJs.ROOT_URL + 'user/addma/check/user',
			timeout : 10000,
			dataType : "json",
			contentType : "text/plain",
			type : "GET",
...
$.validator.addMethod("checkUsername", function(value, element) {
	var regex = /^[a-zA-Z0-9][a-zA-Z0-9_]{5,80}$/;
...
$.validator.addMethod("checkExistUsername", function(value, element) {
	...
	if (!regex.test(username))
		return false;
	if (username) {
		return CMSCommonJs.Form.checkExistUsername(username);
	} else {
		return false;
	}
}, "Tài khoản không tồn tại hoặc không thuộc đơn vị cấp mã!");
